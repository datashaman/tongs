...
class PackageManifest
{
    ...
    /**
     * Get all of the plugins for all packages.
     *
     * @return array
     */
    ...
    public function build()
    {
        ...
        if ($this->files->exists($path = $this->basePath.'/composer.json')) {
            array_push($packages, json_decode($this->files->get($path), true));
        }

        $ignoreAll = in_array('*', $ignore = $this->packagesToIgnore());

        $this->write(collect($packages)->mapWithKeys(function ($package) {
            return [$this->format($package['name']) => $package['extra']['tongs'] ?? []];
    ...
    protected function packagesToIgnore()
    {
        ...
        ), true)['extra']['tongs']['dont-discover'] ?? [];
    }
